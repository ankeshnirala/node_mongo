...
exports.login = catchAsync(async (req, res, next) => {
    ...
    res.status(200).json({
        status: 1,
        message: 'success',
        data: 'your are logged in successfully.',
        token: token
    });
});
...
exports.protect = catchAsync(async (req, res, next) => {
    ...
    if(await user.changedPasswordAfterLogin(decoded.iat)){
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
    ...
    let message = `Forgot your password? submit a PATCH request with your new password to: ${resetURL}.\nIf you didn't forget your password, please ignore this email.`;
...
exports.resetPassword = catchAsync(async (req, res, next) => {
    ...
    let user = await User.findOne({ passwordResetToken: hashedToken, passwordResetExpires: {$gt: Date.now()} });

    // 2) set new password if token has not expired and user exist
    if(!user){
